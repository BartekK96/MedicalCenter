...
export class DoctorService {
  ...
  async login(data: DoctorLoginDTO): Promise<DoctorRO> {
    ...
    const doctor = await this.doctorRepository.findOne({ where: { login } });
  ...
  async register(data: DoctorRegisterDTO): Promise<DoctorRO> {
    const { login } = data;
    let doctor = await this.doctorRepository.findOne({ where: { login } });
    if (doctor) {
    ...
    // check if beforeinsert work(if works delete this two comments line)
